...
await test('Turkish finance input preserves thousands and marks missing values',async()=>{const r=await api.payments.POST(req('boss','/api/payments','POST',{period:'Eylül 2026',ownerName:'Ekrem',bankName:'Test',accountName:'Kart',totalDebt:'1.000',paidAmount:'',monthlyPayment:''}));assert.equal(r.status,201);const row=sql.prepare("SELECT total_debt,paid_amount,missing_fields FROM payment_records WHERE bank_name='Test'").get();assert.equal(row.total_debt,1000);assert.equal(row.paid_amount,0);assert.deepEqual(JSON.parse(row.missing_fields).sort(),['interestDebt','interestRate','minimumPayment','monthlyPayment','nextInstallment','overdraftDebt','overdraftLimit','paidAmount','restructuring','totalLimit'].sort());});
...
await test('bootstrap admin secret rotation restores access and revokes old sessions',async()=>{
 const old=await api.auth.hashPassword('Old-password-123!');
 sql.prepare("INSERT INTO users(id,username,email,full_name,password_hash,password_salt,role) VALUES('admin-reset','admin1','admin1@internal.teksanor','Admin',?,?,'admin')").run(old.hash,old.salt);
 await api.auth.createSession('admin-reset');
 globalThis.__teksanorTestEnv.ADMIN1_PASSWORD='New-password-456!';
 const user=await api.auth.loginWithUsername({username:'admin1',password:'New-password-456!'});
 assert.equal(user.id,'admin-reset');
 assert.equal(sql.prepare("SELECT COUNT(*) AS n FROM sessions WHERE user_id='admin-reset'").get().n,0);
