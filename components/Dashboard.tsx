    ...
    <div className="dashboard-shell">
      ...
      <main className="dashboard-main">
        ...
        <div className="dashboard-content">
          {error && <div className="panel-alert error"><AlertTriangle size={17} />{error}<button onClick={() => setError("")}><X size={16} /></button></div>}
...
function Overview({ data, latest, previous, debtChange, payments, canManage, onDelete }: { data: DashboardData; latest: Summary; previous?: Summary; debtChange: number; payments: Payment[]; canManage: boolean; onDelete: (id: string) => void }) {
  const utilization = latest.total_limit ? (latest.total_debt / latest.total_limit) * 100 : 0;
  const cards = [
    { label: "Toplam borç", value: shortMoney(latest.total_debt), change: debtChange, note: "Önceki döneme göre", icon: CircleDollarSign, tone: "blue" },
    { label: "Aylık ödeme", value: shortMoney(latest.monthly_payment), change: previous ? ((latest.monthly_payment - previous.monthly_payment) / previous.monthly_payment) * 100 : 0, note: "Planlanan ödeme", icon: CreditCard, tone: "gold" },
    { label: "KMH borcu", value: shortMoney(latest.overdraft_debt), change: previous ? ((latest.overdraft_debt - previous.overdraft_debt) / previous.overdraft_debt) * 100 : 0, note: "Ek hesap toplamı", icon: Gauge, tone: "violet" },
    { label: "Asgari ödeme", value: shortMoney(latest.minimum_payment), change: previous ? ((latest.minimum_payment - previous.minimum_payment) / previous.minimum_payment) * 100 : 0, note: "Bu dönem", icon: WalletCards, tone: "green" },
  ];
  return <>
    <section className="stat-grid">{cards.map(({ label, value, change, note, icon: Icon, tone }) => <article className="stat-card" key={label}><div className={`stat-icon ${tone}`}><Icon size={20} /></div><button><MoreHorizontal size={18} /></button><span>{label}</span><strong>{value}</strong><small className={change <= 0 ? "positive" : "negative"}>{change <= 0 ? <ArrowDownRight size={14} /> : <ArrowUpRight size={14} />}{Math.abs(change).toLocaleString("tr-TR", { maximumFractionDigits: 1 })}% <em>{note}</em></small></article>)}</section>
...
function CardTitle({ title, subtitle }: { title: string; subtitle: string }) { return <div className="card-title"><div><h3>{title}</h3><p>{subtitle}</p></div><button><MoreHorizontal size={18} /></button></div>; }
