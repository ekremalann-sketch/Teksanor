...
export async function PUT(request: Request, routeContext: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "Saha ziyareti güncellenemedi." }, { status: errorStatus(error) });
  }
}
...
export async function DELETE(request: Request, routeContext: { params: Promise<{ id: string }> }) {
  ...
  try {
    const context = await requireOrganization(request, user);
    const { id } = await routeContext.params;
    const removed = await getDb().prepare("DELETE FROM field_visits WHERE id = ? AND organization_id = ?").bind(id, context.organization.id).run();
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "Saha ziyareti silinemedi." }, { status: errorStatus(error) });
  }
}
