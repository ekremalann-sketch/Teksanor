...
const COLUMNS = ["visitor_name", "customer_name", "location", "visit_date", "visit_type", "status", "duration_hours", "findings", "actions_taken", "next_visit_date"];
...
async function update(request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await addAudit(user.id, "update", "field_visit", id, "Saha ziyareti güncellendi.", context.organization.id);
