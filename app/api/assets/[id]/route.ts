...
const TABLE = "assets";
const COLUMNS = ["asset_code", "name", "category", "brand", "model", "serial_number", "location", "status", "responsible_name", "purchase_date", "warranty_end", "notes"];
...
async function update(request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await addAudit(user.id, "update", "asset", id, "Varlık güncellendi.", context.organization.id);
...
export async function DELETE(request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await addAudit(user.id, "delete", "asset", id, undefined, context.organization.id);
    return NextResponse.json({ ok: true });
  } catch (error) {
