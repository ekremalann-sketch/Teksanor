...
const COLUMNS = ["name", "description", "trigger_type", "action_type", "is_active"];
...
async function update(request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await addAudit(user.id, "update", "automation", id, "Otomasyon güncellendi.", context.organization.id);
...
export async function DELETE(request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await addAudit(user.id, "delete", "automation", id, undefined, context.organization.id);
    return NextResponse.json({ ok: true });
  } catch (error) {
