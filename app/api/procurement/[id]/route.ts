...
const COLUMNS = ["title", "description", "department", "supplier_name", "quantity", "unit", "unit_price", "total_price", "currency", "status", "priority", "required_date", "notes"];
...
async function update(request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await addAudit(user.id, "update", "procurement", id, "Satın alma talebi güncellendi.", context.organization.id);
...
export async function DELETE(request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await addAudit(user.id, "delete", "procurement", id, undefined, context.organization.id);
    return NextResponse.json({ ok: true });
  } catch (error) {
