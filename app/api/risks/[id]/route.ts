...
const TABLE = "risks";
const COLUMNS = ["title", "description", "department", "category", "likelihood", "impact", "status", "owner_name", "mitigation_plan", "review_date"];
...
async function update(request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await addAudit(user.id, "update", "risk", id, "Risk güncellendi.", context.organization.id);
