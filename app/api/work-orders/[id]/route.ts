...
const COLUMNS = ["title", "description", "customer_name", "location", "department", "order_type", "status", "priority", "assigned_to", "scheduled_date", "estimated_hours", "notes"];
...
async function update(request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await addAudit(user.id, "update", "work_order", id, "İş emri güncellendi.", context.organization.id);
