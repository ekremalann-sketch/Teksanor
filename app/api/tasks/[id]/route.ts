...
const TABLE = "tasks";
const COLUMNS = ["title", "description", "department", "assignee_name", "status", "priority", "due_date"];
...
async function update(request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await addAudit(user.id, "update", "task", id, "Görev güncellendi.", context.organization.id);
